...
def register():
    ...
    if request.method == "POST":
        ...
        if not p_number:
            return apology("brúkaranavn manglar", 403)

        # Ensure password was provided
        if not password:
            return apology("loyniorð manglar", 403)

        # Ensure confirmation was provided
        if not confirmation:
            return apology("endurtak loyniorð", 403)

        # Ensure confirmation matches password
        if password != confirmation:
            return apology("loyniorð mugu samsvara", 403)
